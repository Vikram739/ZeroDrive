...
export default function SignUp() {
  ...
  const inputClass =
    'w-full rounded-md border border-zinc-300 bg-white px-3 py-2.5 text-sm text-zinc-900 outline-none focus:border-zinc-900 focus:ring-2 focus:ring-zinc-900/10 disabled:opacity-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100 dark:focus:border-zinc-50 dark:focus:ring-zinc-50/10'
  const labelClass =
  ...
  const handleSubmit = async (event) => {
    ...
    if (!name.trim()) nextErrors.name = v.nameRequired
    if (!email.trim()) {
      nextErrors.email = v.emailRequired
    } else if (!EMAIL_REGEX.test(email)) {
      nextErrors.email = v.emailInvalid
    }
    if (!password) nextErrors.password = v.passwordRequired
    if (!confirm) {
